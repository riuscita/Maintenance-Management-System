...
function getLatestRecord() {
    echo 'called';
    $uid = $_POST['luid'];
    $sql = "SELECT * FROM request_table WHERE STATUS='WEChecked' ORDER BY RID DESC LIMIT 1";
    $rst = mysql_query($sql);
...
function getLatestDeclinedRecord() {
    $uid = $_POST['luid'];
    $sql = "SELECT * FROM request_table WHERE STATUS='rejected' AND WEID='$uid' ORDER BY RID DESC LIMIT 1";
    $rst = mysql_query($sql);
...
function logRecommendation() {
    ...
    if ($ch == 'accept') {
        ...
        $sql = "UPDATE request_table SET WEID='$uid',STATUS='rejected' WHERE RID='$rid'";
...
if ($action == 'getAllRecords') {
    ...
    getMyHistory();
}else if($action=='getLatestDeclinedRecord') {

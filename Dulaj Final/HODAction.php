...
function getLatestDeclinedRecord() {
    $uid = $_POST['luid'];
    $sql = "SELECT * FROM request_table WHERE STATUS='rejected' AND HODID='$uid' ORDER BY RID DESC LIMIT 1";
    $rst = mysql_query($sql);
...
function getDep($hodid){
    $sql = "SELECT DEPARTMENT FROM user_table WHERE UID=$hodid";
...
function getRequestList() {
    ...
    $dep = getDep($xuid);
    echo $dep;
    $sql = "SELECT request_table.RID,request_table.DES,request_table.DATE,user_table.USER_NAME FROM request_table INNER JOIN user_table ON request_table.UID=user_table.uid WHERE STATUS='submitted' AND request_table.DEPARTMENT='$dep' AND RID>$rid";
    $rst = mysql_query($sql);
...
function logRecommendation() {
    ...
    if ($ch == 'accept') {
        $sql = "UPDATE request_table SET HODID='$uid',STATUS='recommended' WHERE RID='$rid'";
        $msg = "HOD accepted " . $ds;
    } else {
        $sql = "UPDATE request_table SET HODID='$uid',STATUS='rejected' WHERE RID='$rid'";
        $msg = "HOD rejected " . $ds;
    }
